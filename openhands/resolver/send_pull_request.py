...
def make_commit(repo_dir: str, issue: Issue, issue_type: str) -> None:
    ...
    if not result.stdout.strip():
        ...
        subprocess.run(
            f'git -C {repo_dir} config user.name "onitbot" && '
            f'git -C {repo_dir} config user.email "bot@getonit.ai" && '
            f'git -C {repo_dir} config alias.git "git --no-pager"',
            shell=True,
            check=True,
        )
        print('Git user configured as onitbot')
...
def send_pull_request(
    ...
    base_branch_name = f'onitbot-fix-issue-{issue.number}'
